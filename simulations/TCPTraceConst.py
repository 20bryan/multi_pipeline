...
class TCPTraceConst(object):
    ...
    def process_tcptrace_ACK(self, packet, allow_syn=False):
            ...
            ## Case 2.2: Flow record exists and measurement range is open
                ...
                if packet["ackno"] < highest_byte_acked_or_rexmited or packet["ackno"] > highest_expected_ack:
                    ...
                    if actionables_ft2pt[1] in self._tcptrace_packet_table:
                        ...
                        packet_tstamp, packet_seqno = self._tcptrace_packet_table[actionables_ft2pt[1]]
                        rtt = (packet["timestamp"] - packet_tstamp)/timedelta(milliseconds=1)
                        self.duplicate_count += 1
                        self.duplicate_rtts.append(rtt)
        ...
        if len(self.duplicate_rtts) > 0:
            print("rtt excess time is ", sum(self.duplicate_rtts) / len(self.duplicate_rtts))
        ## Case 1: Action is delete
        if actionables_ft2pt[0] == "delete":
            if actionables_ft2pt[1] in self._tcptrace_packet_table:
                # raise ValueError("we caught them")
        ...
        self._flowTableSize.append(len(self._tcptrace_flow_table))
        self._packetTableSize.append(len(self._tcptrace_packet_table))
    ...
    def plot_tcptrace_stats(self, latest_tstamp):
        ...
        def plot_data(x_data, y_data, xlabel, ylabel, title, plot_filename):

            plt.figure(figsize=(6,4))
            time_x = [t/1000 for t in x_data]
            color = next(sns_colors)
            plt.plot(time_x, y_data, color=color, linestyle="-")
            plt.xlabel(xlabel)
            plt.ylabel(ylabel)
            plt.title(title)
            plt.tight_layout()
            plot_path = os.path.join(self._resultsPath, plot_filename)
            plt.savefig(plot_path, dpi=300)
            plt.clf()
            plt.close("all")

        # Plot No. of flow records vs. time
        plot_data(self._snapshotTime, self._intervalActiveFlows, "Time (sec.)", "No. of flow records", "No. of flow records vs. time", "tcptrace_flows_records.png")

        # Plot No. of packet records vs. time
        plot_data(self._snapshotTime, self._intervalActivePackets, "Time (sec.)", "No. of packet records", "No. of packet records vs. time", "tcptrace_packet_records.png")
        print("num entries is ", len(self._intervalActiveFlows))
    ##################################################
